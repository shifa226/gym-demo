    ...
    <>
      <section className="pt-32 md:pt-40 pb-10">
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
          <Reveal>
            ...
            <h1 className="display-heading text-balance">Step Inside VYRA.</h1>
            <p className="text-white/70 text-lg mt-6 max-w-2xl leading-relaxed">
              A look at our premium facility, equipment, and the energy of a VYRA training session.
            </p>
          </Reveal>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-5 lg:px-8">
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-sm font-heading uppercase tracking-wider transition-all duration-300 ${
                    filter === cat ? 'bg-gold-500 text-ink-950' : 'border border-white/10 text-white/60 hover:border-gold-500/50 hover:text-white'
                  }`}
                >
                  {cat}
                </button>
          ...
          {filtered.length === 0 ? (
            <div className="glass-card p-12 text-center">
              <p className="text-white/60">No images in this category yet.</p>
            </div>
          ) : (
            <div className="columns-2 md:columns-3 lg:columns-4 gap-4 [&>*]:mb-4">
              {filtered.map((item, i) => (
                <Reveal key={item.id} delay={i * 50}>
                  <button onClick={() => setLightbox(i)} className="group relative block w-full overflow-hidden rounded-xl">
                    <img src={item.image} alt={item.title} loading="lazy" className="w-full transition-transform duration-500 group-hover:scale-110" />
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-ink-950/95 backdrop-blur-md animate-fade-in p-4" onClick={closeLightbox}>
          ...
          <img src={filtered[lightbox]?.image} alt={filtered[lightbox]?.title} className="max-w-full max-h-[85vh] rounded-xl object-contain" onClick={(e) => e.stopPropagation()} />
